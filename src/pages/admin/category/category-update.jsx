...
export default function CategoryUpdate() {
  ...
  useEffect(() => {
    ...
    fetch(`${pathAdmin}/admin/categories/${id}`, {
      ...
      .then((data) => {
        ...
        setAvatarUrl(cate.avatar || "");
        setBannerUrl(cate.banner || "");
      });
  }, [id]);
  ...
  const validate = () => {
    ...
    // Update: allow keeping existing avatar if user doesn't upload a new file
    if (files.length === 0 && !avatarUrl) {
  ...
  const handlerSubmit = (e) => {
    ...
    if (bannerFiles.length > 0) {
      dataFinal.append("banner", bannerFiles[0]);
    }
    fetch(`${pathAdmin}/admin/categories/${id}`, {
      method: "PATCH",
      headers: {
      ...
      >
        ...
        <div className="mb-[30px] grid sm:grid-cols-2 grid-cols-1 gap-y-[20px] gap-x-[30px] bg-[white] sm:py-[30px] py-[20px] sm:px-[40px] px-[20px] border border-gray-300 rounded-[15px]">
          ...
          <div className="flex flex-col sm:col-span-2 col-span-1">
            ...
            {avatarUrl ? (
              <img
                className="rounded-[10px] w-[160px] mb-[10px] border border-gray-200"
                src={avatarUrl}
                alt="avatar"
              />
            ) : null}
          ...
          <div className="flex flex-col sm:col-span-2 col-span-1">
            <label className="text-[13px] mb-[5px]">Banner</label>
            {bannerUrl ? (
              <img
                className="rounded-[10px] w-full max-w-[520px] mb-[10px] border border-gray-200"
                src={bannerUrl}
                alt="banner"
              />
            ) : null}
            <FilePond
              files={bannerFiles}
              onupdatefiles={(fileItems) => {
                setBannerFiles(fileItems.map((item) => item.file));
              }}
              allowMultiple={false}
              maxFiles={1}
              name="banner"
