...
export default function CollectionDelete({ collection, onDeleted }) {
  ...
  const handleDelete = async () => {
    if (!collection?._id || deleting) return;
    const confirmDelete = window.confirm(
      `Bạn có chắc muốn xoá bộ sưu tập "${collection?.name || ""}"?`
    );
    ...
    try {
      ...
      const response = await fetch(`${pathAdmin}/admin/collections/${collection._id}`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <button
      ...
      title={deleting ? "Đang xoá..." : "Xoá bộ sưu tập"}
    >
