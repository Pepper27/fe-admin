...
export default function RoleDelete({ role, roles, onDeleted }) {
    ...
    const handleDelete = async () => {
        if (!roleData?._id || deleting) return;
        const confirmDelete = window.confirm(
            `Bạn có chắc muốn xoá quyền "${roleData?.name || ""}"?`
        );
        ...
        try {
            ...
            const response = await fetch(`${pathAdmin}/admin/roles/${roleData._id}`, {
                method: "DELETE",
                headers: {
    ...
    return (
        <button
            ...
            title={deleting ? "Đang xoá..." : "Xoá quyền"}
        >
