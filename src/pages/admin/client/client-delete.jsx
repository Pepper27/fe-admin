...
export default function ClientDelete({ client, onDeleted }) {
  ...
  const handleDelete = async () => {
    if (!client?._id || deleting) return;

    const confirmDelete = window.confirm(
      `Bạn có chắc muốn xoá tài khoản khách hàng "${client?.fullName || client?.email || client?._id}"?`
    );
    ...
    try {
      ...
      const response = await fetch(`${pathAdmin}/admin/clients/${client._id}`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <button
      ...
      title={deleting ? "Đang xoá..." : "Xoá tài khoản"}
    >
